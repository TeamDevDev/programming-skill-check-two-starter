...
def test_find_minimum_value():
    ...
    count_executed_lines = run_question_part_a(call_bubble_sort_for_restricted_coverage)
...
def test_generate_fuzzer_values():
    ...
    for char in result:
        assert char_start <= ord(char) < char_start + char_range, "Character is not in range"
